...
const insertDataIntoDB = (sites) => {
    sites.forEach(site => {
        const nom_du_type = site.types.join(" - ");
        const { nom_du_jardin, code_postal, adresse_complete, coordonnees_geographiques, site_internet_et_autres_liens, description } = site;
      ...
      connection.query(query, [nom_du_type, nom_du_jardin, code_postal, site.commune, site.numero_et_libelle_de_la_voie, adresse_complete, lat, lon, siteInternet, description], (error, results) => {
        if (error) {
